...
def start():
	...
	gyro_out = rospy.Publisher('gyro_z_cal', Float32, queue_size=1)
	
	# starting the publishing node for angles
	gyro_out_angle = rospy.Publisher('gyro_z_angle', Float32, queue_size=1)
	
	# starting the publishing node for combined angles
	angle_combined = rospy.Publisher('angle_z_comb', Float32, queue_size=1)
	
	########### START NODES #############################
	
	# starting THIS nodes name
	rospy.init_node('gyro_z_cal_node', anonymous=True)
	
	# subscribing to the spec. gyro node named "gyro_y"
	rospy.Subscriber("gyro_z", Float32, gyro_callback)
	
	# subscribing to the spec. accel node named "accel_y"
	rospy.Subscriber("accel_z", Float32, accel_callback)
