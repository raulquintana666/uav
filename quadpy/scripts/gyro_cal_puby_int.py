...
def start():
	...
	gyro_out = rospy.Publisher('gyro_y_cal', Float32, queue_size=1)
	
	# starting the publishing node for angles
	gyro_out_angle = rospy.Publisher('gyro_y_angle', Float32, queue_size=1)
	
	# starting the publishing node for combined angles
	angle_combined = rospy.Publisher('angle_y_comb', Float32, queue_size=1)
	
	########### START NODES #############################
	
	# starting THIS nodes name
	rospy.init_node('gyro_y_cal_node', anonymous=True)
	
	# subscribing to the spec. gyro node named "gyro_y"
	rospy.Subscriber("gyro_y", Float32, gyro_callback)
	
	# subscribing to the spec. accel node named "accel_y"
	rospy.Subscriber("accel_y", Float32, accel_callback)
	...
	while not rospy.is_shutdown():
		...
		accel_angle = accel_in - accel_offset
    	# make an objet for time and give it the current time
		...
		combined_angle = GYRO_PART * (gyro_angle * GYRO_ANGLE_SCALE_FACTOR + (gyro_rate * delta_t)) + ACC_PART * accel_angle * ACCEL_SCALE_FACTOR
